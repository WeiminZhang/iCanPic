...
*****************************************************************************/
#ifndef GSICQ_HPP_INCLUDED
#define GSICQ_HPP_INCLUDED
...
const int GSIDefaultQueueSize=512;

class GSICQueue :public CQueue
{
public:
  explicit GSICQueue(long buff_size=GSIDefaultQueueSize);
  ...
  errorType Write(const GSIRecord &record);
  ...
  //errorType Read(GSIChar *chr,size_t num,size_t * num_read,long timeout); 
    ...
    errorType Read(char *chr,long timeout){return(CQueue::Read(chr,timeout));}
    errorType Read(char *chr){return(CQueue::Read(chr));}
  //void Flush(void);
